...
class UrlEventHandlers:
    ...
    async def handle_url_shortened(self, event: UrlShortenedEvent) -> None:
        """Handle URL shortened event"""
        try:
            self._logger.info(
                f"Handling URL shortened event for short_code: {event.short_code}"
            )
            await self._analytics_repository.record_url_creation(
            ...
            self._logger.info(
                f"URL shortened event recorded successfully: {event.short_code}"
            )
    ...
    async def handle_url_accessed(self, event: UrlAccessedEvent) -> None:
        ...
        try:
            self._logger.info(
                f"Handling URL accessed event for short_code: {event.short_code}"
            )
            await self._analytics_repository.record_url_access(
                url_id=int(event.url_id),
                short_code=str(event.short_code),
                accessed_at=event.accessed_at,
            )
            self._logger.info(
                f"URL accessed event recorded successfully: {event.short_code}"
            )
        except Exception as e:
